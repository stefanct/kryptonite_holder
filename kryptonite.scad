...
module kryptonite_base() {
  ...
  // cylinder
  color("orange")
    translate([-kryptonite_base_h/2, 0, 0])
      rotate([0,90,0])
        union() {
          intersection() {
  ...
  // hole cover
  color("black")
    tr([-kryptonite_base_h/2+kryptonite_cover_h/2+kryptonite_cover_off, 0, 0],
       [0, 90, 0])
...
module kryptonite_u() {
  ...
  color("black")
    translate([0, 0, u_kryptonite_base_len])
      rotate([180,0,0])
        union() {
          for(i=[-1,1]) {
            translate([i*u_kryptonite_base_dist/2, 0, 0])
